...
// Fetch student feedbacks (only visible)
$stmt = $conn->prepare("
    SELECT f.*, u.name AS student_name
    FROM feedback f
    JOIN student s ON f.student_id = s.student_id
    JOIN user u ON s.student_id = u.user_id
    WHERE f.company_id = ? AND f.is_visible = 1
...
<head>
<meta charset="UTF-8">
<title><?php echo htmlspecialchars($company['company_name']); ?> | Past Feedbacks</title>
<link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<style>
body { margin:0; font-family:'Nunito',sans-serif; background-color:#F5F5F8; }
header { background-color:#9F5EB7; color:white; padding:16px 24px; display:flex; align-items:center; justify-content:space-between; }
...
.sidebar a { padding:12px 20px; margin:8px 16px; text-decoration:none; font-size:16px; color:#f2f2f2; display:flex; align-items:center; gap:10px; border-radius:30px; transition:0.3s; }
...
.company-header { display:flex; align-items:center; gap:16px; background:white; padding:20px; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,0.1); }
.company-header img { width:80px; height:80px; border-radius:10px; object-fit:cover; }
...
/* Feedback List */
.feedback-container { background:white; padding:20px; border-radius:12px; margin-top:20px; }
.feedback-item { padding:12px 0; border-bottom:1px solid #ddd; }
.feedback-item:last-child { border-bottom:none; }
.feedback-item h4 { margin:0 0 6px; color:#9F5EB7; font-size:16px; }
.feedback-item p { margin:4px 0; font-size:14px; color:#333; }
  ...
  <div class="feedback-container">
    <?php foreach($feedbacks as $fb): ?>
      <div class="feedback-item">
        <h4>
          <?php 
            // If anonymous, hide student name
            echo $fb['is_anonymous'] ? 'Anonymous Student' : 'Student: ' . htmlspecialchars($fb['student_name']); 
          ?>
        </h4>
        <p><strong>Rating:</strong> <?php echo htmlspecialchars($fb['rating']); ?> / 5</p>
        <p><strong>Review:</strong> <?php echo nl2br(htmlspecialchars($fb['comment'])); ?></p>
        <small>Submitted on: <?php echo htmlspecialchars($fb['submitted_on']); ?></small>
      </div>
    <?php endforeach; ?>
  </div>
<?php else: ?>
  <p style="text-align:center; color:#666; margin-top:40px;">No reviews yet.</p>
<?php endif; ?>
