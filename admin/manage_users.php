...
// Fetch all users
$stmt = $conn->prepare("SELECT user_id, name, email, role FROM user ORDER BY role, name ASC");
$stmt->execute();
...
<head>
<meta charset="UTF-8">
<title>Manage Users | FYP System</title>
...
<div class="container">
<?php if (empty($users)): ?>
    <p class="empty-msg">There are no users registered yet.</p>
<?php else: ?>
    <table>
        <thead>
            <tr>
        ...
        <?php foreach ($users as $u): ?>
            <tr>
                ...
                <td>
                    <?php if(strtoupper($u['role']) === 'STUDENT'): ?>
                        <a href="view_student.php?user_id=<?php echo urlencode($u['user_id']); ?>">
                ...
                <td>
                    <div class="action-buttons">
                        <a href="delete_user.php?user_id=<?php echo urlencode($u['user_id']); ?>" class="delete-btn" onclick="return confirm('Are you sure you want to delete this user?');">
                            Delete
                        </a>
                    </div>
                </td>
